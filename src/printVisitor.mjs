...
class PrintVisitor extends BIESVMVisitor {
    ...
    visitCstInstruction(ctx) {
      ...
      const toCastMap = new Map([
          [ctx.number(), () => { castingTo = ctx.number().getText(); }],
          [ctx.string(), () => { castingTo = ctx.string().getText(); }],
          [ctx.cstlist(), () => { castingTo = ctx.cstlist().getText(); }]
      ]);
    ...
    visitInoInstruction(ctx) {
      ...
      const instanceMap = new Map([
          [ctx.number(), () => { instanceOff = ctx.number().getText(); }],
          [ctx.string(), () => { instanceOff = ctx.string().getText(); }],
          [ctx.cstlist(), () => { instanceOff = ctx.cstlist().getText(); }]
      ]);
